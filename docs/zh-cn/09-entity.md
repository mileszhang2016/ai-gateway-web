...
- **基本信息**：名称、类型、父Entity、创建时间、更新时间、允许模型、禁止模型
...
- 收集 Key 自身 + 挂载组织 + 全部祖先组织中所有「启用限流」的策略，全部通过才放行；任一策略触发即限流。
...
点击操作列的「管理路由规则」按钮，跳转到路由规则页面，自动筛选出属主为该组织的路由规则。详见 [11 章](11-route.md)。
