...
| 模型服务配置 | 模型协议（至少一种）、模型列表接口路径 |
| 服务鉴权 Keys | 公有云等需鉴权的服务商通常必填 |
| 模型列表 | 点「获取」拉取、输入后回车添加，或「批量添加」粘贴多个名称；**须提交表单才持久化** |
...
模型列表可通过「获取」从上游拉取、在下拉框中输入模型名后按回车添加，或点击「批量添加」一次粘贴多个模型名。
...
点击「详情」打开只读抽屉，展示基本信息（含创建时间、更新时间）、实例池、模型服务配置、服务鉴权 Keys（Key 值脱敏）、模型列表，以及分段计价配置摘要（时区、忙时时间段表；未配置时显示「未配置」）。
