...
> 与 API-Key 允许/禁止模型的组合：Key 的模型访问控制按**重定向后**的目标模型判定。例如原模型名 `glm-5.2-abc`、目标模型 `glm-5.2` 时，Key 允许模型配置 `glm-5.2` 即可放行客户端发送的 `model=glm-5.2-abc` 请求（详见 [10.7 运行时生效机制](10-api-key.md)）。
...
| 字段 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| 负载均衡模式 | 否 | `WRR（加权轮询）` | `WRR`：BFE 本地加权轮询；`EPP`：由 EPP 调度器接管后端选择（实例分配见 [05A 章 EPP 调度](05a-epp-schedule.md)） |
...
- 删除集群前须解除路由规则引用。
- EPP 模式集群的后端选择由 EPP 调度器接管；若实例池中无有效分配（未分配态），该集群导出配置时会**降级为 WRR**。分配关系在「EPP调度」页查看与覆写，见 [05A 章](05a-epp-schedule.md)。
